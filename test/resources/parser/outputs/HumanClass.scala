object H
  ...
  case class Woman(v: Int(32), v: O.Option) extends Human

  case class Man(v: Int(32), v: Int(32)) extends Human

  fn printHuman(human: Human): Unit = {
    human match {
      case Woman(age, timesPregnant) =>
        printWoman(age, timesPregnant)
      case Man(age, nbKids) =>
        printMan(age, nbKids)
      case _ =>
        error("What are you ?")
  ...
  fn printWoman(age: Int(32), timesPregnant: O.Option): Unit = {
    ...
    timesPregnant match {
      case O.Some(nb) =>
        Std.printInt(nb)
      case O.None() =>
        Std.printString("No pregnancies.")
    }
  }
