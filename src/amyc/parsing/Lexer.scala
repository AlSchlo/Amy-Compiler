...
object Lexer extends Pipeline[List[File], Iterator[Token]]
  ...
  val lexer = Lexer(
    ...
    // Identifiers,  
    elem(_.isLetter) ~ many(elem(_.isLetterOrDigit))
    ...
    many1(elem(_.isDigit))
     |> { (cs, range) => Try(cs.mkString.toInt).toOption match {
        case Some(s) => IntLitToken(s).setPos(range._1)
        case None => ErrorToken("Invalid Int") }
      },

    // String literal, 
    word("\"") ~ many(elem(c => c != '\"' && c != '\n')) ~ word("\"")
      |> { (cs, range) => StringLitToken(cs.mkString.slice(1, cs.mkString.length - 1)).setPos(range._1) },

    word("\"") ~ many(elem(c => c != '\"' && c != '\n'))
      |> { (cs, range) => ErrorToken("Unclosed String").setPos(range._1) },
   ...
   word("/*") ~ many(elem(_ != '*')  | (elem('*') ~ elem(_!= '/')))
      |> { (cs, range) => ErrorToken("Unclosed multi-line comments").setPos(range._1) },
