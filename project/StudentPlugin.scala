...
object StudentPlugin extends AutoPlugin {
  ...
  val packageSubmissionZipSettings = Seq(
    packageSubmissionZip := {
      ...
      val binaries = (Compile / packageBin).value
      IO.zip(Seq(sources -> "sources.zip", binaries -> "binaries.jar"), submission, Option.empty)
      submission
    },
    // Exclude resources from binaries
    (Compile / packageBin / mappings) := {
      val relativePaths =
        (Compile / unmanagedResources).value.flatMap(Path.relativeTo((Compile / unmanagedResourceDirectories).value)(_))
